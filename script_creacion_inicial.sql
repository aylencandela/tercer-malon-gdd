...
BEGIN
	...
	INSERT INTO [TERCER_MALON].[caracteristica_x_inmueble]
			   ...
			   ,[id_caracteristica])
	SELECT DISTINCT 
		(SELECT cod_inmueble --PODRIA SIMPLEMENTE PONER EL CODIGO, PERO NO DEBERÍA ESCRBIR UN CODIGO QUE NO EXISTA EN LA BASE DE DATOS A MIGRADA.
				FROM TERCER_MALON.inmueble WHERE cod_inmueble=INMUEBLE_CODIGO),
		(SELECT id_caracteristica
			FROM TERCER_MALON.caracteristica WHERE nombre=SUBSTRING(CARACTERISTICA, LEN('INMUEBLE_CARACTERISTICA_')+1,15))
	FROM gd_esquema.Maestra M
	UNPIVOT
